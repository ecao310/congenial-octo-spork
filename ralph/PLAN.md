...
[ ] Page has gotten long. Split into tabs. For example, tab 1 should be just about the SS 50-85 tax torpedo.
...
[ ] Move the "Other Ordinary Income" slider up with the filing-status/age/SS/muni inputs. The tax-exempt interest readout prices against it but is rendered above it, so the section has to say "the sliders further down"; the Roth section has the same problem with "set above" pointing at a slider two sections up.
[ ] The IRMAA section prices its stat grid against the ordinary-income and LTCG sliders, which live two and three sections below it — the same ordering problem already logged for the tax-exempt interest section. Both go away with one slider move.
