...
[ ] Head of Household is now the only status missing from the segmented control, and the IRMAA table has grown a column per status — a fourth will not fit. Rework `.tier-table` to render only the selected status's thresholds (plus one comparison column) before adding HOH.
