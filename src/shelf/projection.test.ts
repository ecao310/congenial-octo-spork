...
describe('projectYears in real dollars', () => {
  const scenario = {
    ordinaryIncome: 20_000,
    ssBenefit: avgAnnualSSBenefit(2026),
    filingStatus: 'single' as const,
  ...
  // Born 1955, so 71 in 2026, two years short of the applicable age of 73.
  const flat = {
  ...
  it('sums the lifetime real tax out of the rows it reported', () => {
    const p = projectYears(scenario, { ...flat, colaPercent: 2.5 });
