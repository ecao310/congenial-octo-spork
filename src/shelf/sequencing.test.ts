...
describe('a recurring charitable gift', () => {
  const GIFT = { ...ASSUMPTIONS, annualQcd: 15_000 };

  it('is absent from every row when none was asked for', () => {
    for (const id of ALL_IDS) {
      const p = simulateSequencing(id, SCENARIO, ASSUMPTIONS);
      expect(p.totalQcd).toBe(0);
      expect(p.rows.every((row) => row.qcd === 0)).toBe(true);
      // With nothing given, there is nothing between the requirement and the
      // part of it that has to come out as cash.
      expect(p.rows.every((row) => row.rmdAfterQcd === row.rmd)).toBe(true);
    }
    expect(compareSequencing(SCENARIO, ASSUMPTIONS).qcdCurtailed).toBe(false);
  });

  it.each(ALL_IDS)('%s leaves the gift out of the cash it has to spend', (id) => {
    const p = simulateSequencing(id, SCENARIO, GIFT);
    for (const row of p.rows) {
      // The gift never reaches the household, so it is not on either side of
      // the identity — the same three withdrawals fund spending and tax.
      const cash =
        row.ssBenefit +
        row.otherIncome +
        row.muniInterest +
  ...
  it.each(ALL_IDS)('%s grows the gift with inflation and counts it toward the RMD', (id) => {
    const p = simulateSequencing(id, SCENARIO, GIFT);
    p.rows.forEach((row, n) => {
      // Born 1955, so 70 in 2025: the first row gives nothing, because a birth
      // *year* cannot say whether that filer reached 70 1/2 before December.
      expect(row.qcd).toBe(row.age >= 71 ? Math.round(15_000 * 1.025 ** n) : 0);
      // Each figure is rounded from the unrounded one it was computed on, so
      // the difference of two of them can miss the third by a dollar.
      expect(
        Math.abs(row.rmdAfterQcd - Math.max(0, row.rmd - row.qcd)),
      ).toBeLessThanOrEqual(1);
    });
    expect(p.rows[0].qcd).toBe(0);
    expect(p.rows[1].qcd).toBeGreaterThan(0);
    expect(p.firstRmdYear).toBe(2028);
    expect(p.totalQcd).toBe(p.rows.reduce((sum, row) => sum + row.qcd, 0));
  });

  it.each(ALL_IDS)('%s takes the gift out of the IRA before it plans anything', (id) => {
    const p = simulateSequencing(id, SCENARIO, GIFT);
    for (const row of p.rows) {
      // Three roundings, so the identity can miss by that much.
      const closing =
        (row.openingTraditional - row.qcd - row.withdrawnTraditional) * 1.05;
      expect(Math.abs(row.traditional - closing)).toBeLessThanOrEqual(3);
      expect(row.withdrawnTraditional).toBeLessThanOrEqual(
        row.openingTraditional - row.qcd + 1,
      );
    }
  });

  it.each(ALL_IDS)('%s pays less lifetime tax with the gift than without it', (id) => {
    const withGift = simulateSequencing(id, SCENARIO, GIFT);
    const without = simulateSequencing(id, SCENARIO, ASSUMPTIONS);
    expect(withGift.lifetimeRealTax).toBeLessThan(without.lifetimeRealTax);
  });

  it('has all three give the same total while the IRA can fund it', () => {
    const c = compareSequencing(SCENARIO, GIFT);
    expect(c.qcdCurtailed).toBe(false);
    const totals = new Set(c.strategies.map((st) => st.totalQcd));
    expect(totals.size).toBe(1);
    expect(c.strategies[0].totalQcd).toBeGreaterThan(0);
  });

  it('waits for the first age a birth year can settle 70 1/2 at', () => {
    // Born 1960, so 65 in 2025 and not certainly 70 1/2 until 2031.
    const p = simulateSequencing('proportional', SCENARIO, {
      ...GIFT,
      birthYear: 1960,
    });
    expect(p.rows.filter((row) => row.year < 2031).every((row) => row.qcd === 0)).toBe(
      true,
    );
    expect(p.rows.find((row) => row.year === 2031)!.qcd).toBeGreaterThan(0);
  });

  it('reports the orders that ran the IRA down too far to keep giving', () => {
    // Bracket filling exists to empty the IRA early and a recurring gift needs
    // it to still be there, so the two want the same dollars. With $200,000 in
    // the account and $20,000 a year going to charity, the three orders end up
    // giving three different totals, and the cheapest-looking one is cheapest
    // partly because it gave the least.
    const c = compareSequencing(SCENARIO, {
      ...ASSUMPTIONS,
      birthYear: 1950,
      spending: 55_000,
      taxableBalance: 250_000,
      traditionalBalance: 200_000,
      rothBalance: 120_000,
      annualQcd: 20_000,
    });
    expect(c.qcdCurtailed).toBe(true);
    const byId = Object.fromEntries(c.strategies.map((st) => [st.strategy.id, st.totalQcd]));
    expect(byId['bracket-fill']).toBeLessThan(byId['proportional']);
    expect(byId['proportional']).toBeLessThan(byId['taxable-first']);
  });

  it('gives nothing when there is no IRA to give from', () => {
    const p = simulateSequencing('taxable-first', SCENARIO, {
      ...GIFT,
      traditionalBalance: 0,
    });
    expect(p.totalQcd).toBe(0);
